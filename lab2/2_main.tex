...
\subsection{Определение коэффициентов}
Имеем те же формулы для сопряжённых переменных: $\psi_1(t)=A\cos t + B\sin t$, $\psi_2(t)=-A\sin t + B\cos t$. 

\textbf{Шаг 1: Нахождение B.} Для свободного $x_2(0)$ из условий оптимальности получается:
\[
\psi_2(0)=0 \;\Rightarrow\; -A\sin 0 + B\cos 0 = B = 0.
\]

\textbf{Шаг 2: Нахождение C.} Из условия $x_1(0)=0$ и общего решения:
\[
x_1(t) = C\cos t + D\sin t + \tfrac{1}{2}\Big(-\tfrac{A}{2}t\cos t + \tfrac{B}{2}t\sin t\Big)
\]
При $t=0$:
\[
x_1(0) = C\cos 0 + D\sin 0 + \tfrac{1}{2}\Big(-\tfrac{A}{2}\cdot 0\cdot\cos 0 + \tfrac{B}{2}\cdot 0\cdot\sin 0\Big) = C = 0
\]

\textbf{Шаг 3: Система для A и D.} Теперь $B=0$, $C=0$. Из терминальных условий при $t=1$:
\begin{align}
 x_1(1) &= D\sin 1 + \tfrac{1}{2}\Big(-\tfrac{A}{2}\cdot 1\cdot\cos 1\Big) = D\sin 1 - \tfrac{A}{4}\cos 1 = 2,\\
 x_2(1) &= D\cos 1 + \tfrac{1}{2}\Big(-\tfrac{A}{2}(\cos 1 - 1\cdot\sin 1)\Big) = D\cos 1 - \tfrac{A}{4}(\cos 1 - \sin 1) = 0.
\end{align}
...
\textbf{Шаг 4: Решение системы.} Из второго уравнения:
\[
...
\begin{align}
 sc &= \sin 1 \cos 1 \approx 0.4546,\\
 c-s &= \cos 1 - \sin 1 \approx -0.3012,\\
 sc-1 &= 0.4546 - 1 = -0.5454
\end{align}

Получаем:
\begin{align}
 A &= \frac{8c}{sc-1} = \frac{8 \cdot 0.5403}{-0.5454} \approx -7.926,\\
 D &= \frac{2(c-s)}{sc-1} = \frac{2 \cdot (-0.3012)}{-0.5454} \approx 1.104
\end{align}
